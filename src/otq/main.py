...
def command_jupiter(args):
    ...
    paper = not getattr(args, "mainnet", False)
    print(f"🪐 JUPITER MR - {'PAPER' if paper else 'LIVE'}")
    engine = build_jupiter_mr_engine(paper=paper)
    engine.start()
...
def command_jupiter_pairs(args):
    from otq.engines.jupiter_pairs_mr_engine import build_jupiter_pairs_mr_engine

    paper = not getattr(args, "mainnet", False)
    print(f"🪐 JUPITER PAIRS MR (70/30) - {'PAPER' if paper else 'LIVE'}")
    engine = build_jupiter_pairs_mr_engine(paper=paper)
    engine.start()
...
Utilities:
    ...
    parser_jupiter_pairs = subparsers.add_parser("jupiter-pairs", help="Jupiter Pairs MR (70/30 bundle)")
    parser_jupiter_pairs.add_argument("--mainnet", action="store_true", help="Use mainnet (real money)")
    ...
    commands = {
        ...
        "jupiter-pairs": command_jupiter_pairs,
